...
class DataProcessor:
    ...
    def _process_data(self, data: Dict) -> Dict:
        ...
        # 处理点云数据
        if 'point_cloud' in data:
            processed_data['point_cloud'] = self._process_point_cloud(
                data['point_cloud']
            )
            
        # 处理触觉数据
        if 'tactile' in data:
            processed_data['tactile'] = self._process_tactile_data(
                data['tactile']
            )
            
        # 数据增强
        if self.processing_config.get('enable_augmentation', True):
